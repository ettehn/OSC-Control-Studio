(function () {
  ...
  const reportBlocklyBlocksDiagnostic = (kind, message) => {
    try {
      if (window.chrome && window.chrome.webview) {
        window.chrome.webview.postMessage({ kind: 'osccontrol-blockly-diagnostic', level: kind, message });
      } else {
        console.log('[osccontrol-blocks][' + kind + '] ' + message);
      }
    } catch (_) { }
  };
  ...
  function installManualBlockOverrides() {
    Blockly.Blocks.osc_startup_rule = {
      init() {
        ...
        this.setPreviousStatement(true, 'TopLevel');
        this.setNextStatement(true, 'TopLevel');
        this.setColour(190);
        this.setTooltip(zh ? '???????????' : 'Run steps when the app starts.');
        this.setHelpUrl('');
      }
    };
  }
