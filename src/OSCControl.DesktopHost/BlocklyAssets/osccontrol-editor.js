(function () {
  ...
  function initializeEditor() {
    ...
    loadTemplateButton.addEventListener('click', safeLoadSelectedTemplate);
    if (hasWebViewHost()) {
    ...
    setStatus(T('Injecting Blockly workspace...', '正在装载 Blockly 工作区...'));    reportDiagnostic('info', 'blockDef osc_startup_rule=' + typeof (Blockly.Blocks && Blockly.Blocks.osc_startup_rule) + '; init=' + typeof (Blockly.Blocks && Blockly.Blocks.osc_startup_rule && Blockly.Blocks.osc_startup_rule.init));
    ...
    safeLoadTemplate('startup-log', 'default');
  ...
  function safeLoadSelectedTemplate() {
    const select = document.getElementById('templateSelect');
    safeLoadTemplate(select.value, 'manual');
  }
  ...
  function safeLoadTemplate(name, reason) {
    try {
      ...
      updatePreview();
      reportDiagnostic('template', `Failed to load Blockly template: ${name}`, error);
      if (reason === 'manual') {
